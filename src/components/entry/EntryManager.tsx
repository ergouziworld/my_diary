          ...
          {MODES.map((m) => (
            <button
              ...
              className={`flex-1 rounded-lg px-2 py-1.5 text-xs font-medium transition ${
                mode === m.key ? "bg-accent-500 text-white" : "text-slate-400 hover:text-white"
              }`}
            >
              {m.label}
            </button>
          ...
          <div className="space-y-2">
            <div className="flex gap-2">
              ...
              <button
                ...
                onClick={runAiSearch}
                disabled={aiLoading}
                className="shrink-0 rounded-xl bg-accent-500 px-4 py-2 text-sm font-medium text-white transition hover:bg-accent-400 disabled:opacity-60"
              >
                {aiLoading ? "查找中..." : "查找"}
              </button>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 border-t border-white/5 pt-2">
              <button
                ...
                disabled={indexing}
                className="shrink-0 rounded-full border border-accent-500/40 bg-accent-500/10 px-3 py-1.5 text-xs font-medium text-accent-300 transition hover:bg-accent-500/20 disabled:opacity-60"
              >
                {indexing ? "重建中..." : "重建记忆索引"}
              </button>
        ...
        <div className="flex items-center gap-2">
          {!editing && (
            <button
              ...
              className="shrink-0 rounded-lg px-2 py-1 text-xs text-accent-400 transition hover:bg-accent-500/10"
            >
              编辑
            </button>
          )}
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={handleSave}
              disabled={saving}
              className="rounded-full bg-accent-500 px-4 py-2 text-sm font-medium text-white transition hover:bg-accent-400 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {saving ? "保存中..." : "保存"}
            </button>
            <button
