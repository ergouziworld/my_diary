    ...
    <div ref={containerRef} className="flex flex-1 flex-col gap-3 min-h-0">
      ...
      {showLinkInput && (
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={addLink}
            className="rounded-xl bg-accent-500/20 px-3 py-2 text-xs text-accent-400 hover:bg-accent-500/30 transition"
          >
            确认
          </button>
          <button
      ...
      <div className="mt-auto flex min-h-11 shrink-0 items-center justify-between gap-3">
        ...
        <button
          ...
          className="rounded-full bg-accent-500 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-accent-400 disabled:cursor-not-allowed disabled:opacity-60"
        >
          {loading ? "处理中..." : "记录"}
        </button>
      </div>
