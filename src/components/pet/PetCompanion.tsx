    ...
    >
      <div className="relative">
        ...
        <div className={`absolute right-0 ${bubbleBelow ? "top-full mt-2" : "bottom-full mb-2"}`}>
          {open ? (
            <div className="pointer-events-auto w-[min(20rem,calc(100vw-1.5rem))] rounded-2xl border border-accent-500/25 bg-slate-900/95 p-3 shadow-[0_12px_40px_rgba(0,0,0,0.4)] backdrop-blur">
              <div className="mb-2 flex items-center justify-between">
                <span className="text-xs font-medium text-accent-300">{petName}</span>
                <button
                  type="button"
              ...
              <div className="mt-2 flex gap-2">
                ...
                <button
                  ...
                  onClick={handleSend}
                  disabled={loading}
                  className="shrink-0 rounded-xl bg-accent-500 px-3 py-2 text-sm font-medium text-white transition hover:bg-accent-400 disabled:opacity-60"
                >
                  说
                </button>
              </div>
        ...
        <button
          ...
          className={`pointer-events-auto cursor-grab drop-shadow-[0_6px_16px_rgb(var(--accent-500)_/_0.35)] active:cursor-grabbing ${
        ...
        <button
          ...
          aria-label="和宠物聊天"
          className="pointer-events-auto absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full border border-accent-500/40 bg-slate-900 text-xs shadow"
        >
          💬
        </button>
      </div>
